	...
	if ($mysqli->connect_errno) 
{
	...
	$patrolcarDispatched =$POST["chkPatrolcar"];
...
if (!($stml->bind_param('ssssss', $_POST['callerName'],
									$_POST['contactNo'],
									$_POST['incidentType'],
									$_POST['location'],
									$_POST['incidentDesc'],
									$incidentStatus)))
{
	die("Binding parameters failed: ".$stml->errno);
}

	if(!$stmt = $mysqli->execute())
{
	...
	if (!($stmt = $mysqli->prepare($sql)))
{
	...
	if (!($stml->bind_param('s', $patrolcarDispatched[$i])))
{
	die("Binding parameters failed: ".$stml->errno);
}
	if(!$stmt->execute())
{
	...
	$sql = "INSERT INTO dispatch (incidentId, patrolcar_id, time_dispatched) VALUES (?,?, NOW())";
	...
	if (!($stmt = $mysqli->prepare($sql)))
{
	...
	if (!($stml->bind_param('ss', $incidentId,
...
$sql = "SELECT patrolcarId, patrolcar_status_desc FROM patrolcar JOIN patrolcar_status
